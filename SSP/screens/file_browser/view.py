...
from ui.widgets import Header, BackButton, PrimaryButton, SecondaryButton
...
QPushButton {{
    ...
    border: 1px solid {COLORS["border_strong"]}; border-radius: {RADIUS["sm"]}px; height: {BUTTON_HEIGHT}px;
    background-color: {COLORS["bg"]}; padding-left: 12px; padding-right: 12px;
}}
...
PAGINATION_BUTTON_QSS = f"""
QPushButton {{
    color: {COLORS["bg"]}; font-size: 12px; font-weight: 700; border: none;
    border-radius: {RADIUS["sm"]}px; height: {BUTTON_HEIGHT}px;
    background-color: {COLORS["primary"]}; padding-left: 12px; padding-right: 12px;
}}
QPushButton:hover {{ background-color: {COLORS["primary_hover"]}; }}
QPushButton:disabled {{ background-color: {COLORS["border_strong"]}; color: {COLORS["text_muted"]}; }}
"""

NAV_ARROW_QSS = f"""
QPushButton {{
    background-color: {COLORS["primary"]}; color: {COLORS["bg"]}; border: none;
    border-radius: {RADIUS["sm"]}px; font-size: 18px; font-weight: 700;
    min-width: 44px; max-width: 44px; min-height: 44px; max-height: 44px; padding: 0;
}}
QPushButton:hover {{ background-color: {COLORS["primary_hover"]}; }}
"""

# Outline/fill colors for page selection UI (grid thumbnails + single-page view).
# Kept distinct from COLORS['border']/COLORS['border_strong'], which are nearly
# identical light greys and don't read as a clear "selected vs not" outline.
PAGE_OUTLINE_COLOR = "#A8A8A4"
PAGE_SELECTED_COLOR = "#1C1C1A"
CHECKBOX_CHECKED_ICON = icon_path("checkbox-checked").replace("\\", "/")

ZOOM_BUTTON_QSS = f"""
QPushButton {{
    background-color: {COLORS["bg"]}; color: {COLORS["text"]}; border: 1px solid {COLORS["border_strong"]};
    border-radius: {RADIUS["sm"]}px; font-size: 16px; font-weight: 700;
...
QScrollBar:vertical {{
    background-color: {COLORS["bg_subtle"]}; width: 28px; border: none;
    border-radius: {RADIUS["md"]}px; margin: 2px;
}}
QScrollBar::handle:vertical {{
    background-color: {COLORS["border_strong"]}; border: none; border-radius: {RADIUS["md"]}px;
...
class DragScrollArea(QScrollArea):
    """Custom scroll area that supports mouse/touch drag scrolling."""
    ...
    def get_normal_style(self):
        return f"""
            QPushButton {{
                ...
                border-radius: {RADIUS["md"]}px; padding: 10px; text-align: left;
                font-size: 13px; margin: 2px; height: 60px;
            }}
            QPushButton:hover {{ background-color: {COLORS["bg_subtle"]}; border: 1px solid {COLORS["primary"]}; }}
        """
    ...
    def get_selected_style(self):
        return f"""
            QPushButton {{
                ...
                border-radius: {RADIUS["md"]}px; padding: 10px; text-align: left;
                font-size: 13px; font-weight: 700; margin: 2px; height: 60px;
            }}
...
class PDFPageWidget(QFrame):
    ...
    def setup_ui(self, checked):
        self.setStyleSheet(self._checked_style() if checked else self._unchecked_style())
        layout = QVBoxLayout(self)
        layout.setContentsMargins(8, 8, 8, 8)
        layout.setSpacing(6)
        self.checkbox = QCheckBox(f"Page {self.page_num}")
        self.checkbox.setChecked(checked)
        self.checkbox.setMinimumHeight(44)
        self.checkbox.setStyleSheet(f"""
            ...
            QCheckBox::indicator:unchecked {{
                ...
                border: 2px solid {PAGE_OUTLINE_COLOR};
            }}
        """)
        ...
        self.preview_label.setStyleSheet(
            f"QLabel {{ background-color: {COLORS['bg_subtle']}; border: 1px solid {COLORS['border']}; "
            f"border-radius: {RADIUS['sm']}px; color: {COLORS['text_secondary']}; font-size: 10px; }}"
        )
    ...
    def _checked_style(self):
        return (
            f"QFrame {{ background-color: {COLORS['bg']}; border: 2px solid {PAGE_SELECTED_COLOR}; "
            f"border-radius: {RADIUS['md']}px; margin: 4px; }}"
        )

    def _unchecked_style(self):
        return (
            f"QFrame {{ background-color: {COLORS['bg_subtle']}; border: 2px solid {PAGE_OUTLINE_COLOR}; "
            f"border-radius: {RADIUS['md']}px; margin: 4px; }}"
        )
    ...
    def set_error_message(self, error_msg):
        ...
        self.preview_label.setStyleSheet(
            f"QLabel {{ background-color: {COLORS['danger_bg']}; border: 1px solid {COLORS['danger']}; "
            f"border-radius: {RADIUS['sm']}px; color: {COLORS['danger']}; font-size: 9px; }}"
        )
...
class FileBrowserView(QWidget):
    ...
    def setup_ui(self):
        ...
        left_layout.setContentsMargins(10, 16, 10, 20)
        left_layout.setSpacing(10)
        self.file_header = QLabel("PDF Files (0 files)")
        self.file_header.setStyleSheet(
            f"QLabel {{ color: {COLORS['text']}; font-size: {FONT['size_md']}px; font-weight: 700; "
            f"background-color: transparent; padding-left: 13px; }}"
        )
        ...
        # Up/down buttons (hidden - scroll bar is used instead)
        self.file_up_btn = QPushButton("▲ Previous Files")
        self.file_up_btn.setStyleSheet(PAGINATION_BUTTON_QSS)
        self.file_up_btn.setFixedHeight(60)
        self.file_up_btn.setEnabled(False)
        self.file_up_btn.hide()
        file_container_layout.addWidget(self.file_up_btn)

        # Scroll area for file list (drag-scroll enabled for the touchscreen kiosk)
        ...
        file_scroll.setStyleSheet(SCROLLBAR_QSS)
        ...
        self.file_down_btn.setStyleSheet(PAGINATION_BUTTON_QSS)
        self.file_down_btn.setFixedHeight(60)
        self.file_down_btn.setEnabled(False)
        self.file_down_btn.hide()
        ...
        right_panel_layout.setContentsMargins(0, 16, 20, 12)
        ...
        self.preview_header.setStyleSheet(
            f"QLabel {{ color: {COLORS['text']}; font-size: {FONT['size_lg']}px; font-weight: 700; "
        ...
        self.view_all_btn.setStyleSheet(TOGGLE_BUTTON_QSS)
        self.view_single_btn.setStyleSheet(TOGGLE_BUTTON_QSS)
        ...
        self.select_all_btn.setStyleSheet(TOGGLE_BUTTON_QSS)
        ...
        self.deselect_all_btn.setStyleSheet(f"""
            QPushButton {{
                color: {COLORS["bg"]}; font-size: 12px; font-weight: 700;
                border: none; border-radius: {RADIUS["sm"]}px; height: {BUTTON_HEIGHT}px;
                background-color: {COLORS["warning"]}; padding-left: 12px; padding-right: 12px;
            }}
        ...
        self.preview_container.setStyleSheet(
            f"QFrame {{ border: 2px solid {PAGE_OUTLINE_COLOR}; border-radius: {RADIUS['md']}px; "
        ...
        self.preview_layout.setColumnStretch(0, 0)
        self.preview_layout.setColumnStretch(1, 1)
        self.preview_layout.setColumnStretch(2, 1)
        self.preview_layout.setColumnStretch(3, 1)
        self.preview_layout.setColumnStretch(4, 0)
        self.preview_layout.setRowStretch(0, 1)

        # SINGLE PAGE VIEW CONTAINER
        self.single_page_widget = QWidget()
        self.single_page_widget.setStyleSheet(f"background-color: {COLORS['bg_subtle']};")
        ...
        self.prev_grid_page_btn.setStyleSheet(PAGINATION_BUTTON_QSS)
        self.next_grid_page_btn.setStyleSheet(PAGINATION_BUTTON_QSS)
        ...
        self.add_document_button = SecondaryButton("Add Another Document")
        ...
        self.page_info.setStyleSheet(
            f"QLabel {{ color: {COLORS['text']}; font-size: 14px; background-color: transparent; }}"
        )
        self.selected_count_label = QLabel("")
        self.selected_count_label.setStyleSheet(
            f"QLabel {{ color: {PAGE_SELECTED_COLOR}; font-size: 14px; font-weight: 700; "
            f"background-color: transparent; }}"
        )
        ...
        self.prev_page_btn.setStyleSheet(NAV_ARROW_QSS)
        self.next_page_btn = QPushButton("→")
        self.next_page_btn.setStyleSheet(NAV_ARROW_QSS)
        ...
        self.page_input.setStyleSheet(
            f"QLabel {{ background-color: transparent; color: {COLORS['text']}; font-size: 13px; "
            f"min-width: 40px; max-width: 40px; border-radius: {RADIUS['sm']}px; padding: 1px 4px; "
            f"border: none; font-weight: 700; qproperty-alignment: AlignCenter; }}"
        )
        self.single_page_checkbox = QCheckBox("Select this page")
        self.single_page_checkbox.setMinimumHeight(44)
        self.single_page_checkbox.setStyleSheet(
            f"QCheckBox {{ color: {COLORS['text']}; font-size: 13px; background-color: transparent; "
            f"min-height: 44px; }}"
            f"QCheckBox::indicator {{ width: 20px; height: 20px; border-radius: {RADIUS['sm']}px; }}"
            f"QCheckBox::indicator:checked {{ image: url({CHECKBOX_CHECKED_ICON}); "
            f"border: none; background: transparent; }}"
            f"QCheckBox::indicator:unchecked {{ background-color: {COLORS['bg']}; "
            f"border: 2px solid {PAGE_OUTLINE_COLOR}; }}"
        )
        self.single_page_checkbox.stateChanged.connect(
            lambda state: self.single_page_checkbox_clicked.emit(state == Qt.Checked)
        )

        # Zoom controls (single page view only)
        self.zoom_out_btn = QPushButton("−")
        self.zoom_out_btn.setStyleSheet(ZOOM_BUTTON_QSS)
        self.zoom_out_btn.clicked.connect(self.zoom_out_clicked.emit)

        self.zoom_label = QLabel("100%")
        self.zoom_label.setAlignment(Qt.AlignCenter)
        self.zoom_label.setStyleSheet(
            f"QLabel {{ color: {COLORS['text']}; font-size: 13px; font-weight: 700; "
            f"background-color: transparent; min-width: 48px; }}"
        )

        self.zoom_in_btn = QPushButton("+")
        self.zoom_in_btn.setStyleSheet(ZOOM_BUTTON_QSS)
        self.zoom_in_btn.clicked.connect(self.zoom_in_clicked.emit)

        self.zoom_reset_btn = QPushButton("Reset")
        self.zoom_reset_btn.setStyleSheet(TOGGLE_BUTTON_QSS)
        self.zoom_reset_btn.setFixedHeight(36)
        ...
        pagination_controls.addWidget(self.zoom_out_btn, 0, Qt.AlignCenter)
        pagination_controls.addWidget(self.zoom_label, 0, Qt.AlignCenter)
        pagination_controls.addWidget(self.zoom_in_btn, 0, Qt.AlignCenter)
        pagination_controls.addWidget(self.zoom_reset_btn, 0, Qt.AlignCenter)
